...
async function main() {
  console.log("Connecting to Supabase in Mumbai (ap-south-1)...");
  
  // 1. Create Enums & Tables
  await pool.query(`
    DO $$ BEGIN
      CREATE TYPE "ProjectStatus" AS ENUM ('ONGOING', 'UPCOMING', 'COMPLETED');
    EXCEPTION
      WHEN duplicate_object THEN null;
    END $$;

    DO $$ BEGIN
      CREATE TYPE "GalleryCategory" AS ENUM ('PROJECTS', 'INTERIORS', 'AMENITIES', 'CONSTRUCTION');
    EXCEPTION
      WHEN duplicate_object THEN null;
    END $$;
  ...
  console.log("Schema initialized successfully on Supabase!");
  ...
  console.log("SUCCESS! All tables and Ranchi properties migrated to Supabase!");
  await pool.end();
}
